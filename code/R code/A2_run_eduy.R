...
dat <- read.csv("C:/Users/huhu/Desktop/重构code/code/R code/Standard/R data A2/R_vcm_eduy.csv")
...
yp     <- dat[,9] #parent's log income (lnadinc)
ypx    <- dat[,9] #parent's log incodatme (lnadinc)

eduy<- dat[,2] #child's eduy

age <- dat[,3] #child's age
gender <- dat[,4] #gender
urban <- dat[,5] #urban
minzu <- dat[,6] #minzu
mid <- dat[,7] #mid
...
ypz   <- dat[,9] #parent's income (non logged)
ypze   <- exp(dat[,9]) #parent's income (non logged)


basic<-data.frame(yd = yd, ypx=ypx, eduy=eduy, age=age, gender=gender, urban=urban, minzu=minzu, mid=mid, east=east, ypz=ypz, ypze=ypze)
...
age    <- datall[,4] #age
gender     <- datall[,5] #gender
urban    <- datall[,6] #urban
minzu    <- datall[,7] #minzu
mid    <- datall[,8] #mid
east    <- datall[,9] #east

ypz       <- datall[,10] #parent's income (not in logs)
ypze      <- datall[,11] #parent's income (not in logs)
...
#############################################################################################################
x_basic    <- data.frame(ypx=ypx, age=age, eduy=eduy,gender=gender, urban=urban, minzu=minzu, mid=mid, east=east)
########################################################################################################################
###############    Estimation of IGM   #########################################
################################################################################
...
###########################################################################
################ Linear model
lm_model <- lm(yd~ypx+eduy+age+gender+urban+minzu+mid+east,data=yxz_dat)
summary(lm_model)
...
bw_cv <- npscoefbw(yd~ypx+eduy+age+gender+urban+minzu+mid+east|ypz, data=yxz_dat, nmulti=min(5,ncol(x_dat)+1))
bw <- npscoefbw(yd~ypx+eduy+age+gender+urban+minzu+mid+east|ypz, data=yxz_dat, nmulti=min(5,ncol(x_dat)+1),bandwidth.compute=FALSE,bws=2.5*bw_cv$bw)
...
for(b in 1:num_boot) {
	...
	lm_model_boot <- lm(yd~ypx+eduy+age+gender+urban+minzu+mid+east,data=yxz_dat)
...
d1_2_boot <- matrix(NA,9,num_boot)
d1_2_boot <- matrix(NA,9,num_boot)
d1_3_boot <- matrix(NA,9,num_boot)
d1_4_boot <- matrix(NA,9,num_boot)
d1_5_boot <- matrix(NA,9,num_boot)
d2_3_boot <- matrix(NA,9,num_boot)
d2_4_boot <- matrix(NA,9,num_boot)
d2_5_boot <- matrix(NA,9,num_boot)
d3_4_boot <- matrix(NA,9,num_boot)
d3_5_boot <- matrix(NA,9,num_boot)
d4_5_boot <- matrix(NA,9,num_boot)
