...
dat <- read.csv("C:/Users/huhu/Desktop/重构code/code/R code/Standard/R data Son2/R_vcm_eduy.csv")
...
basic<-data.frame(yd = yd, ypx=ypx, eduy=eduy, age=age, urban=urban, minzu=minzu, mid=mid, east=east, ypz=ypz, ypze=ypze)
...
#############################################################################################################
x_basic    <- data.frame(ypx=ypx, age=age, eduy=eduy, urban=urban, minzu=minzu, mid=mid, east=east)
########################################################################################################################
###############    Estimation of IGM   #########################################
################################################################################
...
###########################################################################
################ Linear model
lm_model <- lm(yd~ypx+eduy+age+urban+minzu+mid+east,data=yxz_dat)
summary(lm_model)
...
bw_cv <- npscoefbw(yd~ypx+eduy+age+urban+minzu+mid+east|ypz, data=yxz_dat, nmulti=min(5,ncol(x_dat)+1))
bw <- npscoefbw(yd~ypx+eduy+age+urban+minzu+mid+east|ypz, data=yxz_dat, nmulti=min(5,ncol(x_dat)+1),bandwidth.compute=FALSE,bws=2.5*bw_cv$bw)
...
for(b in 1:num_boot) {
	...
	lm_model_boot <- lm(yd~ypx+eduy+age+urban+minzu+mid+east,data=yxz_dat)
...
write.csv(IGM,file = "IGM_Son2.csv")
