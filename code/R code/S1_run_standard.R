...
dat <- read.csv("C:/Users/huhu/Desktop/重构code/code/R code/Standard/R data Son1/R_vcm.csv")
...
basic<-data.frame(yd = yd, ypx=ypx, age=age, urban=urban, minzu=minzu, mid=mid, east=east, ypz=ypz, ypze=ypze)
...
age    <- datall[,3] #age
urban    <- datall[,4] #urban
minzu    <- datall[,5] #minzu
mid    <- datall[,6] #mid
east    <- datall[,7] #east

ypz       <- datall[,8] #parent's income (not in logs)
ypze      <- datall[,9] #parent's income (not in logs)
...
#############################################################################################################
x_basic    <- data.frame(ypx=ypx, age=age, urban=urban, minzu=minzu, mid=mid, east=east)
########################################################################################################################
###############    Estimation of IGM   #########################################
################################################################################
...
###########################################################################
################ Linear model
lm_model <- lm(yd~ypx+age+urban+minzu+mid+east,data=yxz_dat)
summary(lm_model)
...
bw_cv <- npscoefbw(yd~ypx+age+urban+minzu+mid+east|ypz, data=yxz_dat, nmulti=min(5,ncol(x_dat)+1))
bw <- npscoefbw(yd~ypx+age+urban+minzu+mid+east|ypz, data=yxz_dat, nmulti=min(5,ncol(x_dat)+1),bandwidth.compute=FALSE,bws=2.5*bw_cv$bw)
...
for(b in 1:num_boot) {
	...
	lm_model_boot <- lm(yd~ypx+age+urban+minzu+mid+east,data=yxz_dat)
...
d1_2_boot <- matrix(NA,7,num_boot)
d1_2_boot <- matrix(NA,7,num_boot)
d1_3_boot <- matrix(NA,7,num_boot)
d1_4_boot <- matrix(NA,7,num_boot)
d1_5_boot <- matrix(NA,7,num_boot)
d2_3_boot <- matrix(NA,7,num_boot)
d2_4_boot <- matrix(NA,7,num_boot)
d2_5_boot <- matrix(NA,7,num_boot)
d3_4_boot <- matrix(NA,7,num_boot)
d3_5_boot <- matrix(NA,7,num_boot)
d4_5_boot <- matrix(NA,7,num_boot)
...
write.csv(IGM,file = "IGM_Son1.csv")
